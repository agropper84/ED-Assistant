...
export async function POST(request: NextRequest) {
  try {
    const { text, operation, hint, context } = await request.json();
...
Rewrite ONLY the selected text with more detail incorporated. Keep the same clinical voice and style. Do not add information that wouldn't be known from the context. Output ONLY the rewritten text, nothing else.`;
...
${context ? `Surrounding context from the same section:\n${context}\n` : ''}Make this more concise while preserving all clinically important information. Remove unnecessary words and redundancy. Keep the same professional tone. Output ONLY the shortened text, nothing else.`;
    } else {
